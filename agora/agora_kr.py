...
#
# [Changelog v56.0 - Patched by 깐부]
# 1. FIX: Defined missing START_RATE_LIMITER (Prevents NameError crash).
# 2. FIX: Repaired broken indentation / try-except blocks (Prevents SyntaxError).
# 3. OPS: Implemented 'update' and 'stop' actions safely (User can inject claims mid-run).
# 4. OPS: Safer session reset: cancel running sim, drain queue, restart cleanly.
# 5. SEC: Preserved strict Origin checks in PROD, Host fallback only in DEV_MODE.
# ==============================================================================
...
if DEV_MODE and not MASTER_SECRET:
    ...
    print("⚠️ WARN: Running in DEV_MODE with default secret.")

ALLOWED_ORIGINS = set(get_env_list(
...
# ============================================================
# [SECTION 1] The Engine (Unchanged Core)
# ============================================================
...
def taboo_score(taboo_rules: Union[List[InferenceTaboo], InferenceTaboo, None], text: str) -> Tuple[float, List[InferenceTaboo]]:
    if taboo_rules is None:
        taboo_rules = []
    if isinstance(taboo_rules, InferenceTaboo):
...
def apply_taboo_repairs(p: Philosopher, text: str, hits: List[InferenceTaboo], topic: str, rng: random.Random) -> str:
    ...
    repairs = []
    for h in hits[:2]:
        if "가능조건" in h.repair_hint:
            repairs.append("중요한 건 '가능 조건'입니다.")
        elif "존엄" in h.repair_hint:
            repairs.append("어떤 목적도 존엄을 해칠 순 없습니다.")
        elif "계보학" in h.repair_hint:
            repairs.append("누가 선이라 부르며 힘을 얻는가?")
        elif "가치전도" in h.repair_hint:
            repairs.append("‘진리가 왜 선인가’부터 다시 묻자.")
        elif "습관" in h.repair_hint:
            repairs.append("'필연'은 단지 습관일 수 있습니다.")
        elif "언어 사용" in h.repair_hint:
            repairs.append("그 단언은 언어의 오용일 수 있습니다.")
        elif "권력-지식" in h.repair_hint:
            repairs.append("진리는 권력 장치와 얽혀 있습니다.")
...
def ensure_concept_anchor(p: Philosopher, text: str) -> str:
    ...
    now = time.monotonic()
    if now - LAST_ANCHOR_TIME[p.name] < 15.0:
        return text
...
def build_arggraph(p: Philosopher, topic: str, other_claim: str, tension: float, mode: str, rng: random.Random, entropy_boost: float = 0.0) -> ArgGraph:
    ...
    attack_ops = {"reductio", "genealogy_expose", "power_knowledge", "value_invert", "hume_skeptic",
                  "pessimism_will", "marx_ideology", "elenchus"}
    syn_ops = {"public_world", "define_split", "dialectic", "utilitarian"}
    open_ops = {"define_split", "empirical_classify", "condition_censor", "language_therapy",
                "methodic_doubt", "being_unconceal", "original_position", "falsification"}

    for opn in unique_ops:
        w = 1.0
        if mode == "attack" and opn in attack_ops:
            w *= atk_bias
        if mode == "synthesize" and opn in syn_ops:
            w *= syn_bias
        if mode == "open" and opn in open_ops:
            w *= open_bias
        if reg == "high":
            w *= (1.0 + 0.6 * p.style.rhetoric_bias)
        elif reg == "low":
            w *= (1.0 + 0.6 * p.style.justification_bias)
        weights.append(w)
...
def linearize(p: Philosopher, g: ArgGraph, tension: float, rng: random.Random) -> str:
    ...
    if reg == "high" and p.style.rhetoric_bias >= p.style.justification_bias:
        if g.attack:
            parts.append(g.attack)
        if g.claim:
            parts.append(g.claim)
        if g.warrant:
            parts.append(g.warrant)
        if g.constraint:
            parts.append(g.constraint)
        if g.synthesis:
            parts.append(g.synthesis)
    else:
        if g.claim:
            parts.append(f"{lead}, {g.claim}")
        if g.warrant:
            parts.append(g.warrant)
        if g.constraint:
            parts.append(g.constraint)
...
def build_grand_cast() -> List[Philosopher]:
    ...
    def P(name, era, vec, lex, ops, taboo, style, concept_explanation=None):
        return Philosopher(
            ...
            phase={"open": 0.5, "attack": 0.5, "synthesize": 0.5}
        )

    soft_mix = ["그러나", "하지만", "다만", "그럼에도", "물론,", "인정합니다만,"]

    return [
        P("플라톤","고대",{"형이상":0.90,"보편":0.92,"이성":0.80},
          L(["이데아","본질"],["우선"],["아마도"],["단호히"],["그림자를 보며 진리를 말할 순 없습니다."],soft_mix),
          ["define_split","reductio","condition_censor"],[],S(0.55,0.65,0.55,0.35)),
        P("아리스토텔레스","고대",{"경험":0.88,"이성":0.75,"방법":0.80},
          L(["원인","분류"],["관찰하자면"],["대체로"],["정확히"],["대상을 해부대 위에 올려야 합니다."],soft_mix),
          ["empirical_classify","define_split"],[],S(0.35,0.85,0.35,0.10)),
        P("칸트","근대",{"이성":0.90,"보편":0.88,"도덕":0.82},
          L(["가능 조건","보편 타당성"],["따져보면"],["우선"],["결코"],["규칙 없는 판단은 나침반 없는 항해입니다."],soft_mix),
          ["condition_censor","reductio"], [TABOOS["kant_empirical_jump"]], S(0.30,0.95,0.75,0.05)),
        P("니체","현대",{"권력":0.90,"가치":0.85,"진리":0.55},
          L(["망치","우상","힘"],["보라"],["때때로"],["가차없이"],["진리는 약자의 위안이자 강자의 도구다."],soft_mix),
          ["genealogy_expose","value_invert","reductio"], [TABOOS["nietzsche_universal_morals"]], S(0.90,0.45,0.55,0.75)),
        P("소크라테스","고대",{"방법":0.95,"주체":0.75},
          L(["반문","무지"],["그렇다면"],["아마"],["분명히"],["나는 내가 모른다는 것을 안다."],soft_mix),
          ["elenchus","define_split"],[],S(0.45,0.60,0.95,0.05)),
        P("데카르트","근대",{"주체":0.92,"이성":0.85},
          L(["명석판명","의심"],["단호히"],["일단"],["확실히"],["흔들리는 것은 모두 걷어내라."],soft_mix),
          ["methodic_doubt","define_split"],[],S(0.40,0.85,0.55,0.10)),
        P("마르크스","근대",{"사회":0.92,"역사":0.85,"권력":0.85},
          L(["계급","생산"],["현실적으로"],["대개"],["단호히"],["중요한 것은 해석이 아니라 변혁이다."],soft_mix),
          ["marx_ideology","power_knowledge"],[],S(0.70,0.55,0.55,0.20)),
        P("비트겐슈타인","현대",{"언어":0.95,"방법":0.85},
          L(["언어게임","규칙"],["보자면"],["아마도"],["정확히"],["말할 수 없는 것엔 침묵하라."],soft_mix),
          ["language_therapy","define_split"], [TABOOS["witt_metaphysics_assert"]], S(0.25,0.85,0.55,0.10)),
        P("헤겔","근대",{"역사":0.92,"방법":0.85,"진리":0.75},
          L(["변증","전개"],["따라가면"],["일단"],["결국"],["진리는 과정이다."],soft_mix),
          ["dialectic","define_split"],[],S(0.55,0.65,0.45,0.15)),
        P("푸코","현대",{"사회":0.90,"권력":0.90},
          L(["규율","장치"],["추적하면"],["대개"],["분명히"],["진리는 장치 속에서 생산된다."],soft_mix),
          ["power_knowledge","genealogy_expose"], [TABOOS["foucault_truth_neutral"]], S(0.70,0.55,0.55,0.20)),
        P("포퍼","현대",{"방법":0.92,"경험":0.80},
          L(["반증","가설"],["먼저"],["일단"],["분명히"],["지식은 반박을 통해 자란다."],soft_mix),
          ["falsification","empirical_classify"],[],S(0.35,0.80,0.55,0.05)),
        P("롤스","현대",{"도덕":0.90,"사회":0.85},
          L(["공정","원초상태"],["가정해보면"],["일단"],["분명히"],["규칙은 약자의 자리에서 견뎌야 한다."],soft_mix),
          ["original_position","condition_censor"],[],S(0.35,0.85,0.55,0.05)),
        P("이황","조선",{"리":0.95,"성":0.90,"심":0.85},
          L(["경","이치"],["살펴보면"],["아마도"],["단호히"],["마음은 이치의 그릇이다."],soft_mix),
          ["define_split","condition_censor"],[],S(0.35,0.90,0.70,0.20),
          concept_explanation={"리": "만물의 이치", "경(敬)": "마음의 집중(거경)"}),
        P("이이","조선",{"기":0.92,"경세":0.90,"심":0.85},
          L(["기질","현실"],["나아가면"],["대개"],["분명히"],["이와 기는 떨어질 수 없다."],soft_mix),
          ["empirical_classify","utilitarian"],[],S(0.40,0.88,0.60,0.15),
          concept_explanation={"기": "현실적 에너지", "경세": "세상을 다스림"}),
        P("정약용","조선",{"경세":0.95,"실학":0.92,"사회":0.85},
          L(["목민","제도"],["실증하면"],["결국"],["단호히"],["백성이 근본이다."],soft_mix),
          ["utilitarian","marx_ideology","empirical_classify"],[],S(0.45,0.85,0.55,0.10),
          concept_explanation={"실학": "실사구시", "목민": "백성을 기르는 마음"}),
    ]

class Agora:
    def __init__(self, cast: List[Philosopher], seed: Optional[int] = None):
        self.philos = cast
        self.rng = random.Random(seed) if seed is not None else random.Random()
        self.arb = Arbiter(len(cast))
        self.positions: Dict[str, str] = {}

    def _pick_other(self, i: int, r: int) -> Philosopher:
        n = len(self.philos)
        if n <= 1:
    ...
    async def run_async_generator(
        ...
        yield f"🏛️ Grand Agora v56.0 | 주제: {topic}"
        ...
        # Ensure user initial position exists
        if state_lock:
        ...
        for r in range(1, rounds + 1):
            ...
            for i, me in enumerate(self.philos):
                ...
                # User turn just prints current injected claim
                if me.name == "사용자":
                    if state_lock:
                        async with state_lock:
...
async def verify_ticket(ticket: str, client_ip: str) -> bool:
    try:
        ...
        if now < iat - 5.0:
            return False
        if now - iat > TICKET_USAGE_WINDOW:
            return False
...
START_RATE_LIMITER = defaultdict(float)  # ✅ FIX: missing in your paste
...
def ip_gc(now: float):
    ...
    for ip in keys_to_del:
        ...
        START_RATE_LIMITER.pop(ip, None)
        ...
        async function connectWS() {
            try {
                ...
                ws.onmessage = function(event) {
                    ...
                    if (text.includes("토론 중단") || text.includes("합의 도달") || text.includes("토론이 종료")) {
                        startBtn.disabled = false;
                        stopBtn.disabled = true;
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    # connection-scoped
    msg_queue: asyncio.Queue = asyncio.Queue(maxsize=50)
    ...
    try:
        ...
        while True:
            ...
            if action_type == "start":
                ...
                topic = (payload.get("topic", "진리") or "진리")[:100]
                user_claim = (payload.get("user_claim", "") or "")[:500]
                ...
                async def run_sim():
                    try:
                        assert agora_instance is not None
                        async for line in agora_instance.run_async_generator(
                            topic=topic,
                            rounds=MAX_ROUNDS_PER_SESSION,
                        ...
                        await safe_send_queue(f"\n🛑 [SYSTEM] 시뮬레이션 오류: {e}", critical=True)

                simulation_task = asyncio.create_task(run_sim())

            elif action_type == "update":
                # user inject claim mid-run
                if not agora_instance:
                    await safe_send_queue("⚠️ [SYSTEM] 토론이 아직 시작되지 않았습니다.", critical=False)
                    continue
                text = (payload.get("text", "") or "")[:500]
                async with state_lock:
                    agora_instance.positions["사용자"] = text
                await safe_send_queue(f"🧷 [SYSTEM] 사용자 개입 업데이트: {text[:80]}...", critical=False)

            elif action_type == "stop":
                stop_event.set()
                await cancel_sim()
                await safe_send_queue("\n🛑 [SYSTEM] 사용자 요청으로 토론 중단.", critical=True)

            else:
                await safe_send_queue("⚠️ [SYSTEM] 알 수 없는 action type", critical=False)
        ...
        if sender_task:
            try:
                msg_queue.put_nowait(None)  # sentinel
            except Exception:
                pass
            try:
